...
Original file is located at
    https://colab.research.google.com/drive/1ZD3-nSqmI0rwrUROcoGDFHSvEPpj7zBo
"""
...
def possible_downsample_linear(x, in_channels, out_channels):
...
class RevBlockFunctionLinear(Function):
    @staticmethod
    ...
    @staticmethod
    def _forward(x, in_channels, out_channels, training, f_params, f_buffs, g_params, g_buffs,
                 no_activation=False):
        ...
        with torch.no_grad():
            ...
            x1_ = possible_downsample_linear(x1, in_channels, out_channels)
            x2_ = possible_downsample_linear(x2, in_channels, out_channels)

            f_x2 = RevBlockFunctionLinear.residual(
                x2,
                in_channels,
                out_channels,
                f_params,
            ...
            g_y1 = RevBlockFunctionLinear.residual(
                y1,
                out_channels,
                out_channels,
                g_params,
    ...
    def _backward(output, in_channels, out_channels, f_params, f_buffs,
        ...
        with torch.no_grad():
            ...
            x2 = y2 - RevBlockFunctionLinear.residual(
                y1,
                out_channels,
                out_channels,
                g_params,
            ...
            x1 = y1 - RevBlockFunctionLinear.residual(
                x2,
                in_channels,
                out_channels,
                f_params,
    ...
    @staticmethod
    def _grad(x, dy, in_channels, out_channels, training, activations, f_params, f_buffs, g_params, g_buffs,
    ...
    @staticmethod
    def forward(ctx, x, in_channels, out_channels, training, no_activation, activations, storage_hooks, *args):
        ...
        f_params = [Variable(x) for x in args[:4]]
        g_params = [Variable(x) for x in args[4:8]]
        f_buffs = args[8:10]
        g_buffs = args[10:]
        ...
        y = RevBlockFunctionLinear._forward(
            x,
            in_channels,
            out_channels,
            training,
    ...
    def backward(ctx, grad_out):
        ...
        f_params = [Variable(p, requires_grad=True) for p in saved_tensors[:4]]
        g_params = [Variable(p, requires_grad=True) for p in saved_tensors[4:8]]
        ...
        if ctx.load_input:
            ...
            x = RevBlockFunctionLinear._backward(
                output,
                in_channels,
                out_channels,
        ...
        dx, dfw, dgw = RevBlockFunctionLinear._grad(
            x,
            grad_out,
            in_channels,
            out_channels,
        ...
        #print(dfw.shape,dgw.shape,dx.shape)
        return ((dx,None,None,None,None,None,None,None,None,None,None) + tuple(dfw) +
                tuple(dgw) + tuple([None]*4))


class RevBlockLinear(nn.Module):
    def __init__(self, in_channels, out_channels, activations, no_activation=False, storage_hooks=[]):
        super(RevBlockLinear, self).__init__()
    ...
    def reset_parameters(self):
        ...
        self._buffers['g_rm1'].zero_()
        self._buffers['g_rv1'].fill_(1)        


    def forward(self, x):
        return RevBlockFunctionLinear.apply(
            x,
            self.in_channels,
            self.out_channels,
            self.training,
